...
clear direc
suffixmask='*.mat';
direc=dir([directory,filesep,suffixmask]);datanames={};
[datanames{1:length(direc),1}]=deal(direc.name);
datanames=sortrows(datanames);
% datanames(882:length(datanames))=[];
amountmask=length(datanames);
clear direc 
...
s{5,1}= 'ROI';                   s{5,2}=[700,1,800,1079];         % Region of interest: [x,y,width,height] in pixels (matrix indexing), may be left empty
...
for i=1:1:amount-inc
    ...
    image2=imread(fullfile(directory, filenames{i+inc}));
    ...
    if isempty(datanames)==0
        load(fullfile(directory,datanames{i}));
    %     mask=maske.msk;
